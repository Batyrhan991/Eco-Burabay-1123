...
/* =========================================================
   ECO BURABAY — script.js (Оптимизированная версия)
   ========================================================= */
...
async function loadGlobalData() {
  ...
  try {
    ...
    if (!sightsResponse.error && sightsResponse.data && sightsResponse.data.length) {
      SIGHTS = sightsResponse.data;
    }
    if (!treesResponse.error && treesResponse.data && treesResponse.data.length) {
...
function updateCounters() {
  ...
  animateCounter(tEl, TREES.length);
  animateCounter(sEl, SIGHTS.length);
  animateCounter(rEl, reports.length + 2);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. Привязываем закрытие модалок по оверлею сразу
  ...
  // 2. Инициализируем интерфейс СРАЗУ (Белого экрана больше не будет)
  initBurger();
  initAuth();
  renderSights();
  renderTrees();
  ...
  // 3. Асинхронно подтягиваем данные из облака Supabase в фоне
  loadGlobalData().then(() => {
    // Мягко обновляем UI актуальными данными из базы, если они загрузились
    renderSights();
    renderTrees();
    updateCounters();
    handleRouting();
